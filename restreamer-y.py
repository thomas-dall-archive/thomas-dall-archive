...
class RestreamerService:
    ...
    def _get_direct_stream_url(
        self, youtube_url: str
    ) -> "tuple[Optional[str], Optional[str]]":
        """Checks YouTube live status using yt-dlp and returns the direct URL.

        Returns (resolved_stream_url, video_watch_url). `video_watch_url` is
        the actual https://www.youtube.com/watch?v=... page for the specific
        broadcast that was resolved -- useful for notifications, since the
        input `youtube_url` passed in is often a channel's /live URL rather
        than the specific video. Both are None on failure.

        On an unclassified failure (not one of the known "not live" phrases),
        retries once immediately with verbose extractor output before falling
        back to the caller's normal idle-poll wait. This catches the common
        startup race where a just-started broadcast briefly returns "No video
        formats found!" before YouTube's backend finishes tagging it live.
        """
        self.logger.info(f"Checking live status for → {youtube_url}")

        def _build_cmd(verbose: bool) -> list:
            cmd = [
            ...
            cmd += ["--verbose"] if verbose else ["--no-warnings"]
            cmd += [
        ...
        def _attempt(verbose: bool) -> tuple:
            """Runs yt-dlp once.

            Returns (resolved_url, watch_url, is_unclassified_failure).
            """
            cmd = _build_cmd(verbose)
            ...
            if result.returncode == 0:
                ...
                # Prefer yt-dlp's own webpage_url; fall back to building it
                # from the video id if that field is ever absent.
                watch_url = video_metadata.get('webpage_url')
                if not watch_url and video_metadata.get('id'):
                    watch_url = f"https://www.youtube.com/watch?v={video_metadata['id']}"
                ...
                return resolved_url, watch_url, False
            ...
            # Unclassified failure -- this is the "No video formats found!"
            # bucket that's often just a broadcast-startup race.
            if verbose:
                self.logger.error(
                    f"yt-dlp verbose retry also failed (exit {result.returncode}). "
                    f"Full trace below for diagnosis:\n{result.stderr.strip()}"
                )
            else:
                self.logger.warning(
                    f"yt-dlp failed (exit {result.returncode}): "
                    f"{result.stderr.strip()!r} -- retrying once with -v in 5s "
                    f"in case this is a broadcast-startup race."
                )
            return None, None, True

        try:
            resolved_url, watch_url, retry_worthy = _attempt(verbose=False)
            if resolved_url or not retry_worthy:
                return resolved_url, watch_url

            time.sleep(5)
            resolved_url, watch_url, _ = _attempt(verbose=True)
            return resolved_url, watch_url
    ...
    def _check_for_stream_signals(
            ...
            try:
                ...
                if messages:
                    ...
                    for msg in messages[:6]:
                        ...
                        send_discord_embed(
                            "live", "📬 New Stream Signal (Wix)",
                            f"Video ID: `{video_id}`\n🔗 [Watch on YouTube]({target_url})",
                            0x3498db
                        )

                        candidate, _ = self._get_direct_stream_url(target_url)
        ...
        if stream_url is None and streams.get("youtube_url"):
            ...
            if now >= self._next_fallback_check_at:
                ...
                candidate, watch_url = self._get_direct_stream_url(streams["youtube_url"])
                ...
                if candidate:
                    ...
                    # watch_url is the specific video yt-dlp resolved to,
                    # e.g. https://www.youtube.com/watch?v=abc123 -- distinct
                    # from streams["youtube_url"], which is just the
                    # channel's /live redirect link.
                    display_url = watch_url or streams["youtube_url"]
                    send_discord_embed(
                        "live", "⚡ Fallback Recovery",
                        f"Direct polling found live stream\n🔗 [Watch on YouTube]({display_url})",
                        0x2ecc71
                    )
        ...
        try:
            while self.process.poll() is None:
                ...
                if refresh_at and time.time() >= refresh_at and source_url:
                    ...
                    candidate, _ = self._get_direct_stream_url(source_url)
                    if candidate:
                ...
                if (api.get("rumble_stats_url")
                    ...
                    if stream_age is not None:
                        ...
                        if age_hours >= self.MAX_STREAM_AGE_HOURS:
                            ...
                            send_discord_embed(
                                "alerts",
                                "🔴🔴 RUMBLE STREAM NEEDS MANUAL END 🔴🔴",
                                f"**The output stream just hit the {self.MAX_STREAM_AGE_HOURS}h age limit.**\n\n"
                                f"👉 Go end it now: https://rumble.com/account/live-streaming",
                                0xFF0000,
                                mention="<@1505349451810734140>"
                            )
